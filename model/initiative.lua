...
Initiative:add_reference{
  ...
  selector_generator = function(list, options)
    ...
    sub_selector:left_join("interest", nil, { "interest.issue_id = issue.id AND interest.member_id = ?", options.member_id })
    sub_selector:add_field("interest.member_id NOTNULL", "own_participation")
    sub_selector:add_field("interest.member_id", "participating_member_id")
    ...
    selector:left_join("supporter", nil, { "supporter.initiative_id = initiative.id AND supporter.member_id = ?", options.member_id })

    selector:left_join("direct_supporter_snapshot", "supporter_s", { "supporter_s.event = issue.latest_snapshot_event AND supporter_s.initiative_id = initiative.id AND supporter_s.member_id = ?", options.member_id })
