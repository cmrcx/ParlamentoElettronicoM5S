  ...
  if initiative.issue.fully_frozen and initiative.issue.closed and initiative.admitted then
    ...
    ui.container{
      ...
      content = function()
        ...
        local function abs(votes)
          slot.put("<b>" .. votes .. "</b>")
        end

        local function perc(votes, sum)
          if sum > 0 then
            slot.put(" (" .. string.format( "%.f", votes * 100 / sum ) .. "%)")
          end
        end
        ...
        slot.put(_"Yes" .. ": ")
        abs(initiative.positive_votes)
        perc(initiative.positive_votes, sum_votes)
        slot.put(" &nbsp;&middot;&nbsp; ")
        slot.put(_"Abstention" .. ": ")
        abs(initiative.issue.voter_count - sum_votes)
        slot.put(" &nbsp;&middot;&nbsp; ")
        slot.put(_"No" .. ": ")
        abs(initiative.negative_votes)
        perc(initiative.negative_votes, sum_votes)
        slot.put(" &nbsp;&middot;&nbsp; <b>")
